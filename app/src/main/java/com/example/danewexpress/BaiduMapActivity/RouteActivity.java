...
import com.example.danewexpress.utils.BikingRouteOverlay;
import com.example.danewexpress.utils.DrivingRouteOverlay;
import com.example.danewexpress.utils.TransitRouteOverlay;
import com.example.danewexpress.utils.WalkingRouteOverlay;
    ...
    OnGetRoutePlanResultListener listener = new OnGetRoutePlanResultListener() {
        ...
        public void onGetWalkingRouteResult(WalkingRouteResult result) {
            ...
            if (result.error == SearchResult.ERRORNO.AMBIGUOUS_ROURE_ADDR) {
                ...
                mEtStart.setText(suggestAddrInfo.getSuggestStartNode().get(0).address);
                mEtEnd.setText(suggestAddrInfo.getSuggestEndNode().get(3).address);

                Log.d("*", suggestAddrInfo.getSuggestStartNode().get(0).address);
                Log.d("*", suggestAddrInfo.getSuggestStartNode().get(1).address);
                Log.d("*", suggestAddrInfo.getSuggestStartNode().get(2).address);
                Log.d("*", suggestAddrInfo.getSuggestStartNode().get(3).address);

                Log.d("*", suggestAddrInfo.getSuggestEndNode().get(0).address);
                Log.d("*", suggestAddrInfo.getSuggestEndNode().get(1).address);
                Log.d("*", suggestAddrInfo.getSuggestEndNode().get(2).address);
                Log.d("*", suggestAddrInfo.getSuggestEndNode().get(3).address);
                return;
            }
...
//            }
        ...
        public void onGetDrivingRouteResult(DrivingRouteResult result) {
            ...
            if (result.error == SearchResult.ERRORNO.AMBIGUOUS_ROURE_ADDR) {
                // 起终点或途经点地址有岐义，通过以下接口获取建议查询信息
                SuggestAddrInfo suggestAddrInfo = result.getSuggestAddrInfo();
                Log.d("*", suggestAddrInfo.getSuggestEndNode().toString());
                /*Toast.makeText(RouteActivity.this, "你好，请检查起点或终点信息，建议输入起点为："
                        + suggestAddrInfo.getSuggestStartNode() + "建议输入起点为：" +
                        suggestAddrInfo.getSuggestEndNode(), Toast.LENGTH_SHORT).show();*/
                mEtStart.setText(suggestAddrInfo.getSuggestStartNode().get(0).address);
                mEtEnd.setText(suggestAddrInfo.getSuggestEndNode().get(0).address);
                Log.d("*", suggestAddrInfo.getSuggestEndNode().get(0).address);
                return;
            }
            if (result.error == SearchResult.ERRORNO.NO_ERROR) {
